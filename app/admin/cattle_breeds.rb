ActiveAdmin.register CattleBreed do
  ...
  permit_params :cattle_category_id, :title, :created_by, :updated_by, :description
  ...
  form do |f|
    f.inputs do
      f.input :cattle_category_id, as: :select, :prompt => "Select Cattle Category", collection: CattleCategory.all.collect {|c_cat| [c_cat.title, c_cat.id] }
      f.input :title
      f.input :description
